...
`git clone`부터 핸드오프 문서 요약, 프로젝트 공유(복사+커밋+push)까지 직접 할 필요 없이, 자기 AI 코딩 에이전트(Claude Code·Codex·Cursor 등 무엇이든)에게 아래처럼 한 줄만 시켜도 됩니다. 이 저장소의 [AGENTS.md](AGENTS.md)에 자세한 절차가 적혀 있어서, 그 파일을 읽을 줄 아는 에이전트라면 알아서 따라합니다.
